...
def evaluate(test_nodes, raw_features, labels, USTGCN, regression, test_loss):
...
class TrafficModel:
    ...
    def __init__(self, train_data, train_label, test_data, test_label, adj,
                 ...
                 epochs, num_timestamps, pred_len, save_flag, PATH, t_debug, b_debug):
    ...
    def run_model(self):
        ...
        timeStampModel = CombinedGNN(self.input_size, self.out_size, self.adj,
                                      1, self.GNN_layers, self.num_timestamps, self.day)
        ...
        for epoch in range(1, epochs):
            ...
            for data_timestamp in idx:
                ...
                pred = pred + temp_predicts.detach().tolist()  #228个预测
...
def apply_model(train_nodes, CombinedGNN, regression,
                node_batch_sz,train_data, train_label, avg_loss, lr):
...
hModel = TrafficModel(train_data, train_label, test_data, test_label, adj, config, ds, input_size,
                      out_size, GNN_layers, epochs,  num_timestamps, pred_len, save_flag,
